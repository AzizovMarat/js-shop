...
const DOC = document;
const CART = [];
const NUMBER_OF_RPODUCTS_IN_CART = [];
...
function createNewProductCard(index, productType, productName, author, price, img) {
    ...
    newProduct.setAttribute("class", "col-xl-6");
    newProduct.innerHTML = `<div
                                ...
                                <div class="col p-4 d-flex flex-column">
                                    <div class="row-auto d-flex flex-column">
                                        ...
                                        <div class="d-flex align-items-end">${price} ₽</div>
                                    </div>
                                </div>
    
                                <div class="col-auto d-none d-md-block">
                                    <img class="bd-placeholder-img" src="${img}" alt="${productName}" width="299" height="299">
                                </div>
                            </div>`;
...
function createProductContent(index, productType, productName, author, price, img, about) {
    productContent.innerHTML = `<div
                                class="row g-0 border rounded overflow-hidden flex-md-row h-md-250 position-relative">
...
function createProductInCart(index, productName, price, count) {
    let newProduct = DOC.createElement("div");
    newProduct.setAttribute("class", "row");
    newProduct.setAttribute("id", `${index}`);
    newProduct.innerHTML = `<div class="col border rounded border-light-dark bg-light-dark text-truncate">
...
function addInCart(index) {
    let indx = CART.indexOf(products[index]);

    if (CART == null || CART.length == 0 || indx == -1) {
        CART.push(products[index]);
        NUMBER_OF_RPODUCTS_IN_CART.push(1);
        createProductInCart(index, products[index].productName, products[index].price, NUMBER_OF_RPODUCTS_IN_CART[NUMBER_OF_RPODUCTS_IN_CART.length - 1]);
    } else {
        NUMBER_OF_RPODUCTS_IN_CART[indx] = NUMBER_OF_RPODUCTS_IN_CART[indx] + 1;
        DOC.getElementById(index).querySelector("#count").innerHTML = NUMBER_OF_RPODUCTS_IN_CART[indx];
    }
}
...
function removeFromCart(index) {
    ...
    if (indx > -1) {
        CART.splice(indx, 1);
        NUMBER_OF_RPODUCTS_IN_CART.splice(indx, 1);
    }
